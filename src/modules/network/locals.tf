...
locals {
  ...
  workers_ingress = [
    ...
    {
      description = "Allow stateful ingress to DB Mysql.",
      protocol    = local.tcp_protocol,
      port        = -1,
      source      = var.hop_database_cidr,
      source_type = "CIDR_BLOCK",
      stateless   = false
    },
    {
      description = "Allow stateful ingress to HSM.",
      protocol    = local.tcp_protocol,
      port        = -1,
      source      = var.hop_hsm_cidr,
      source_type = "CIDR_BLOCK",
      stateless   = false
    },
    {
      description = "Allow stateful ingress to workers. Required for load balancer http/tcp health checks",
      protocol    = local.tcp_protocol,
    ...
    {
      description = "allow stateful ingress that allows ssh access to the worker nodes from the Operator MS",
      protocol    = local.tcp_protocol,
      port        = local.ssh_port,
      source      = "10.72.13.0/30",
      source_type = "CIDR_BLOCK",
      stateless   = false
    },    
  ...
  int_lb_egress = [
    ...
    },
    {
      ...
      destination_type = "CIDR_BLOCK",
      protocol         = local.tcp_protocol,
      port_min         = local.health_check_port,
      port_max         = local.health_check_port,
      stateless        = false
    },
  ]
  int_lb_ingress = []
