...
module "bastion" {
  ...
  label_prefix = format("%s-%s", var.label_prefix, terraform.workspace)

  # networking
  availability_domain = var.availability_domains["bastion"]
  bastion_access      = var.bastion_access
  ig_route_id         = lookup(var.ig_route_id, terraform.workspace)
  netnum              = lookup(var.subnets["bastion"], "netnum")
  newbits             = lookup(var.subnets["bastion"], "newbits")
  vcn_id              = lookup(var.vcn_id, terraform.workspace)
...
module "operator" {
  ...
  compartment_id = lookup(var.compartment_id, terraform.workspace)
  label_prefix   = format("%s-ms-%s", var.label_prefix, terraform.workspace)

  # networking
  availability_domain = var.availability_domains["operator"]
  nat_route_id        = lookup(var.nat_route_id, terraform.workspace)
...
module "network" {
  ...
  compartment_id = lookup(var.compartment_id, terraform.workspace)
  label_prefix   = var.label_prefix
  vcn_cidrs      = lookup(var.vcn_cidrs, terraform.workspace)
  ...
  #Custom variables for hop payshield project

  # usado para as regras de acesso entre as subredes
  hop_database_cidr = lookup(var.hop_database_cidr, terraform.workspace)

  hop_hsm_cidr = var.hop_hsm_cidr
